...
from models.spectral_layer import spectral_dense
...
class RealNVP(bijector.Bijector):
  ...
  def _forward(self, x):
    ...
    if log_scale is not None:
      #log_scale =math_ops.tanh(log_scale) #TODO
      y1 *= math_ops.exp(log_scale)
    if shift is not None:
      y1 += shift
    y = array_ops.concat([x0, y1], axis=-1)
    with tf.name_scope('summaries'):
      r = math_ops.exp(log_scale)
      with tf.name_scope("max_log_scale"):
        tf.summary.scalar('max_log_scale', tf.reduce_max(r))
      with tf.name_scope("min_log_scale"):
        tf.summary.scalar('min_log_scale', tf.reduce_min(r))
      with tf.name_scope("max_shift"):
        tf.summary.scalar('max_shift', tf.reduce_min(shift))
      with tf.name_scope("min_shift"):
        tf.summary.scalar('min_shift', tf.reduce_min(shift))
    return y
  ...
  def _inverse_log_det_jacobian(self, y):
    ...
    #log_scale = math_ops.tanh(log_scale)  # TODO
    with tf.name_scope('summaries'):
      r = -math_ops.reduce_sum(log_scale, axis=-1)
      with tf.name_scope("max_log_det"):
        tf.summary.scalar('max_log_det', tf.reduce_max(r))
      with tf.name_scope("min_log_det"):
        tf.summary.scalar('min_log_det', tf.reduce_min(r))
...
def real_nvp_spectral_template(
    hidden_layers,
    shift_only=False,
  ...
  with ops.name_scope(name, "real_nvp_spectral_template"):
    ...
    return template_ops.make_template(
        "real_nvp_spectral_template", _fn)
